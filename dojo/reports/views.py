    ...
    def get(self, request: HttpRequest) -> HttpResponse:
        try:
            ...
            if response.status_code == 200:
                ...
                external_html = "<p>Failed to load external content.</p>"

        except requests.exceptions.RequestException as e:
            external_html = f"<p>Error: {e}</p>"

            # Now pass it into your normal Django template
        add_breadcrumb(title="Report Builder", top_level=True, request=request)
        return render(request, self.get_template(), {'external_html': external_html})

    def get_template(self):
        return "dojo/wordreport_builder.html"
